...
#import "MyGuidesViewController-obs.h"
...
@property (nonatomic, weak) UIView *localLoadingView;
...
-(void)applicationBecameActive:(NSNotification *)notification
{
   // NSLog(@"application became active MyGuidesVC %@", self.objects);
    self.totalGuidesInDatabase = [self retreiveTotalGuideCount];
    ...
    for (UIView *subview in self.view.subviews)
    {
        if ([subview class] == NSClassFromString(@"PFLoadingView"))
        {
            ...
            for (UIView *loadingViewSubview in subview.subviews) {
                ...
                {
                    self.localLoadingView = loadingViewSubview;
                ...
                {
                    self.activityIndicatorView = (UIActivityIndicatorView *)loadingViewSubview;
...
- (void)objectsDidLoad:(NSError *)error {
    ...
    // This is a workaround for a parse framework problem where the loading indicator gets stuck on the first load
    if (!self.isLoading) {
...
//-(UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  //  if ([object isKindOfClass:[PFGuide class]]) {
     //   PFGuide *guideToDisplay = (PFGuide *)object;
        PFGuide *guideToDisplay = (PFGuide *)[self.objects objectAtIndex:indexPath.row];
...
-(PFTableViewCell *)tableView:(UITableView *)tableView cellForNextPageAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // Configure the cell
    cell.textLabel.text = @"Load more guides...";
...
/*
// For DEBUG ONLY - REMOVE AFTER CRASH BUG FIXED
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    if (section == 0) {
        NSLog(@"NUMBER OF ROWS IN SECTION %d", [self.objects count]);
...
- (IBAction)loadMoreButtonPressed:(UIButton *)sender {
    ...
    if ( (![self isLoading]) && ([self.objects count] < self.totalGuidesInDatabase) ) {
        int page = (self.totalGuidesInDatabase / [self.objects count]);
        [self loadObjects:page clear:NO];
    }
    else {
        // have loaded all objects so hide loadMoreButton
...
-(NSUInteger)retreiveTotalGuideCount
{
    ...
    [query countObjectsInBackgroundWithBlock:^(int number, NSError *error) {
        if (!error) {
            totalGuides = number;
            if ([self.objects count] < totalGuides)
            {
